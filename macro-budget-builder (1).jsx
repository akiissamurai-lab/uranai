...
/*
 * ═══════════════════════════════════════════════════════════════
 *  マクロ飯ビルダー v3 — Full Feedback Integration
 *  
 *  Changes from v2:
 *  - [BUG] AI JSON parse: try-catch + fallback
 *  - [BUG] Slider/input conflict: number input primary, slider secondary
 *  - [ALG] Category diversity + randomization in solver
 *  - [FEAT] Gender input (Mifflin-St Jeor accuracy)
 *  - [FEAT] Individual food exclusion toggles
 *  - [FEAT] PFC ideal range + calculation basis transparency
 *  - [FEAT] Share via clipboard
 *  - [FEAT] History via persistent storage
 *  - [UX] Proper step collapse/accordion
 *  - [UX] AI loading doesn't block other tabs
 *  
 *  Production notes (for future deployment):
 *  - Move API call to backend (Next.js API route / Edge Function)
 *  - Split into components: ProfileStep, ParamsStep, ResultView
 *  - Add TypeScript interfaces for FoodItem, MealPlan, AIAdvice
 * ═══════════════════════════════════════════════════════════════
 */
...
function solveMealPlan(budget, targetProtein, targetCal, excludedIds, catExclusions) {
  ...
  // Category limits to prevent monotony
  const catLimits = { meat: 3, fish: 2, egg: 4, soy: 3, dairy: 2, grain: 3, veg: 2, fruit: 2, supp: 2 };
  const catCount = {};

  // Score with slight randomness (±15%) for variety on regeneration
  ...
  // Phase 1: protein with category limits
  ...
  // Phase 3: ensure at least 1 veggie
...
function calcBMR(weight, height, age, gender) {
  ...
  return weight * 24; // simple fallback
}
...
{"personalMessage":"2-3文のパーソナルメッセージ（性別・年齢・BMI・体脂肪率・期限すべて考慮）","meals":[{"timing":"朝食","name":"料理名","emoji":"絵文字","ingredients":"食材","recipe":"3ステップ以内","macros":"P:○g F:○g C:○g ○kcal"},{"timing":"昼食","name":"...","emoji":"...","ingredients":"...","recipe":"...","macros":"..."},{"timing":"夕食","name":"...","emoji":"...","ingredients":"...","recipe":"...","macros":"..."},{"timing":"間食","name":"...","emoji":"...","ingredients":"...","recipe":"...","macros":"..."}],"weeklyTip":"週間戦略2-3文","warning":"注意点またはnull","productTips":"年齢・性別に合わせたおすすめ商品アドバイス1-2文"}`;
  ...
  try {
    ...
    // Fallback: try cleaning common issues
...
export default function App() {
  ...
  // State
  const [step, setStep] = useState("profile"); // profile | params | result
  ...
  const handleGenerate = useCallback(async () => {
    ...
    saveHistory(entry).then(setHistory);
      ...
      <main style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 100px" }}>
        ...
        {step === "result" && result && (
          <div ref={resultRef} style={{ animation: "fadeUp 0.5s ease-out" }}>
            ...
            {/* Tabs — accessible even during AI loading */}
